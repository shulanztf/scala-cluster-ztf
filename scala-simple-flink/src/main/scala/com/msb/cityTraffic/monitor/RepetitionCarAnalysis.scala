...
import com.msb.util.{RepetitionCarWarning, SinkInfo, TrafficInfo, WriteDataSink}
...
/**
 * @description: 套牌车分析，从数据流中找出存在套牌嫌疑的车辆.
 * @author: zhaotf
 * @create: 2020-07-05 11:23
 */
...
//    props.setProperty("group.id","msb_001")
   ...
   val result:DataStream[RepetitionCarWarning] = stream.keyBy(_.car) //根据车牌号分
...
//      result.print()
//      result.addSink(new WriteDataSink[RepetitionCarWarning](classOf[RepetitionCarWarning]))
    result.addSink(new SinkInfo())
