...
object Demo01 {
  def main(args: Array[String]): Unit = {
    ...
    props.setProperty("bootstrap.servers", "hserver133:9092")
    props.setProperty("group.id", "flink-kafka-001")
    props.setProperty("key.deserializer", classOf[StringSerializer].getName)
    props.setProperty("value.deserializer", classOf[StringSerializer].getName)

    val stream = env.addSource(new FlinkKafkaConsumer[String]("flink-kafka-1", new SimpleStringSchema(), props))
    stream.map(data => {
      ...
      .process(new ProcessAllWindowFunction[(String, Long), String, TimeWindow] {
        ...
        override def process(context: Context, elements: Iterable[(String, Long)], out: Collector[String]): Unit = {
          ...
          println("最大车速的车牌号：" + maxSpeedCarInfo+"|"+ sortList.last._2+ "\t最小车速的车牌号：" + minSpeedCarInfo+"|"+sortList.head._2)
          for (elem <- sortList) {
            out.collect(elem._1)
          }
        }
