...
object ReadingFromKafkaTomcat2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val textRslt:DataStream[JSONObject] = transaction.map(x=> {
      val arr = StringUtils.split(x," ")
    ...
    textRslt.print()
    val dr = textRslt.map(_.toJSONString)
    dr.print()
    ...
    env.execute("flink-tomcat-kafka-1")
  }
...
//    kafkaProps.setProperty("max.poll.records", "2") //
